...
def _log(msg: str, level: str = "INFO"):
    ...
    try:
        ...
        console.print(f"[yellow]Log write failed: {exc}[/yellow]")
...
class Indicators:
    ...
    def ingest_text(self, text: str, region: str = "US"):
        ...
        for m in IPV4_RE.findall(text):
            try:
                # Quick sanity, keep only valid IPv4 addresses
                socket.inet_aton(m)  # raises OSError if invalid
                self.ips.add(m)
            except OSError:
                continue
...
class DarkelfCLI:
    ...
    def cmd_scan(self):
        ...
        # 📱 Phone handling
            ...
            try:
                ...
                _log(f"Phone indicator error: {exc}", "WARN")
    ...
    def _reverse_dns_lookup(self):
        ...
        lines.append("[bold cyan]Hostnames:[/bold cyan]")
    ...
    def _darkelf_ai_run(
        ...
        try:
            # nosec B603 - Safe subprocess:
            # - executable resolved via shutil.which()
            # - model validated against a whitelist
            # - shell=False
            # - prompt is passed only to stdin
            proc = subprocess.run(
            ...
            raise RuntimeError(exc.stderr.strip() or "Ollama execution failed.") from exc
        except OSError as exc:
            raise RuntimeError(f"Failed to launch Ollama: {exc}") from exc
    ...
    def _scribe_select_model(self, choice: str) -> str:
        ...
        if psutil:
            try:
                ...
                _log(
                    f"Unable to determine system RAM ({exc}); defaulting to mistral.",
                    "WARN",
                )
